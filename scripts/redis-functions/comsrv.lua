...
local function comsrv_batch_update(keys, args)
    ...
    redis.call('HSET', timestamp_key,
        'last_update', redis.call('TIME')[1],
        'last_update_type', telemetry_type,
        'last_update_count', update_count
    )

    -- Publish update event for subscribers
    redis.call('PUBLISH', string.format("comsrv:update:%s:%s", channel_id, telemetry_type), cjson.encode({
    ...
    if telemetry_type == "T" or telemetry_type == "S" then
        ...
        redis.call('PUBLISH', 'alarmsrv:check', cjson.encode({
            source = 'comsrv',
            channel_id = channel_id,
            telemetry_type = telemetry_type,
            points = updates
...
local function comsrv_command_trigger(keys, args)
    ...
    -- Generate command ID
    local command_id = string.format("%s_%s_%d_%d",
        channel_id,
        command_type,
        redis.call('TIME')[1],
    ...
    redis.call('HINCRBY', string.format("comsrv:stats:%s", channel_id),
        command_type == "C" and "control_commands" or "adjustment_commands", 1)
...
local function comsrv_get_four_telemetry(keys, args)
    ...
    if args[1] then
        ...
        if decoded then telemetry_types = decoded end
    end
...
local function comsrv_server_handle_write(keys, args)
    ...
    redis.call('HSET', meta_key,
        'last_write', redis.call('TIME')[1],
        'last_write_type', telemetry_type,
        'last_write_address', address
    )

    -- Publish write event
    redis.call('PUBLISH', string.format("comsrv:write:%s:%s", channel_id, telemetry_type), cjson.encode({
        channel_id = channel_id,
        telemetry_type = telemetry_type,
        address = address,
        value = value,
...
local function comsrv_sync_channels(keys, args)
    ...
    redis.call('HSET', meta_key,
        'last_sync', redis.call('TIME')[1],
        'sync_source', source_channel,
        'sync_points', synced_points
    )
