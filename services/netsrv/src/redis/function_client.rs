...
impl RedisFunctionClient {
    ...
    pub async fn collect_and_forward(
        ...
        // First collect data
        let collected_data = self.collect_data(source_pattern, batch_size, "hash").await?;
        
        // Extract the data array
        let data = collected_data
            .get("data")
            .ok_or_else(|| NetSrvError::Data("Missing data field in collection result".to_string()))?;
