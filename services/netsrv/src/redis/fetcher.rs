...
impl OptimizedDataFetcher {
    pub async fn new(config: RedisConfig, poll_interval_secs: u64, batch_size: usize) -> Result<Self> {
    ...
    pub async fn fetch_data(&mut self) -> Result<Value> {
        ...
        for pattern in &self.config.data_keys {
            ...
            match self.client.collect_data(pattern, self.batch_size, "hash").await {
                Ok(result) => {
    ...
    pub async fn get_stats(&mut self) -> Result<NetworkStats> {
        ...
        Ok(NetworkStats {
            total_forwarded: stats.get("forwarded")
                .and_then(|v| v.as_u64())
                .unwrap_or(0),
            total_failed: stats.get("failed")
                .and_then(|v| v.as_u64())
                .unwrap_or(0),
            total_queues: stats.get("total_queues")
                .and_then(|v| v.as_u64())
                .unwrap_or(0),
...
mod tests {
    ...
    async fn test_optimized_fetch() {
        ...
        let mut fetcher = OptimizedDataFetcher::new(config, 1, 100)
            .await
            .unwrap();
