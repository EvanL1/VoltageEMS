...
impl NetworkClient for LegacyMqttClient {
    ...
    async fn send(&self, data: &str) -> Result<()> {
        if let Some(ref client) = self.client {
            ...
            client
                .publish(&self.config.topics.publish, qos, false, data)
                .await
                .map_err(|e| {
            ...
            debug!(
                "Published to topic {}: {}",
                self.config.topics.publish, data
            );
        }
        Ok(())
    }
    ...
    fn setup_aws_iot_options(&self, mqttoptions: &mut rumqttc::MqttOptions) -> Result<()> {
        ...
        if let crate::config_new::CloudProvider::Aws = self.config.provider {
            ...
            if self
                ...
                .tls
                .alpn_protocols
                .contains(&"x-amzn-mqtt-ca".to_string())
            {
    ...
    async fn send(&self, data: &str) -> Result<()> {
        if let Some(ref client) = self.client {
            ...
            client
                ...
                .await
                .map_err(|e| {
                    crate::error::NetSrvError::Network(format!("Cloud MQTT publish failed: {}", e))
                })?;
