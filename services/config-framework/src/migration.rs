...
impl ConfigMigrator {
        ...
        T: Serialize,
    {
        ...
        let source_value: S = match self.source_path.extension().and_then(|e| e.to_str()) {
            Some("yaml") | Some("yml") => serde_yaml::from_str(&source_content)
                .map_err(|e| ConfigError::Parse(e.to_string()))?,
            Some("toml") => {
                toml::from_str(&source_content).map_err(|e| ConfigError::Parse(e.to_string()))?
            }
            Some("json") => serde_json::from_str(&source_content)
                .map_err(|e| ConfigError::Parse(e.to_string()))?,
        ...
        let target_content = match self.target_path.extension().and_then(|e| e.to_str()) {
            Some("yaml") | Some("yml") => serde_yaml::to_string(&target_config)
                .map_err(|e| ConfigError::Parse(e.to_string()))?,
            Some("toml") => toml::to_string_pretty(&target_config)
                .map_err(|e| ConfigError::Parse(e.to_string()))?,
            Some("json") => serde_json::to_string_pretty(&target_config)
                .map_err(|e| ConfigError::Parse(e.to_string()))?,
            _ => return Err(ConfigError::Parse("Unsupported file format".into())),
        };
    ...
    pub fn backup(&self) -> Result<PathBuf> {
        let backup_path = self.source_path.with_extension(format!(
            "{}.backup",
            self.source_path
                .extension()
                .and_then(|e| e.to_str())
...
pub mod transformers {
    use super::*;
    use crate::base::{
        BaseServiceConfig, LoggingConfig, MonitoringConfig, RedisConfig, ServiceInfo,
    };
        ...
        Ok(BaseServiceConfig {
            service: ServiceInfo {
                ...
                description: env_map
                    ...
                    .cloned()
                    .unwrap_or_default(),
                instance_id: env_map.get("INSTANCE_ID").cloned().unwrap_or_default(),
            },
            redis: RedisConfig {
                url: env_map
                    .get("REDIS_URL")
            ...
            monitoring: MonitoringConfig {
                metrics_enabled: env_map
                    .get("METRICS_ENABLED")
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(true),
                metrics_port: env_map
                    .get("METRICS_PORT")
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(9090),
                health_check_enabled: env_map
                    .get("HEALTH_CHECK_ENABLED")
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(true),
                health_check_port: env_map
                    .get("HEALTH_CHECK_PORT")
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(8080),
                health_check_interval: env_map
    ...
    pub fn merge_with_base(base: BaseServiceConfig, service_config: Value) -> Result<Value> {
        ...
        if let (Some(result_obj), Some(service_obj)) =
            (result.as_object_mut(), service_config.as_object())
        {
            ...
            let url = obj
                ...
                .or_else(|| {
                    obj.get("redis_host").map(|host| {
                        ...
                        Value::String(format!(
                            "redis://{}:{}",
                            host.as_str().unwrap_or("localhost"),
                            port
                        ))
                    })
                })
