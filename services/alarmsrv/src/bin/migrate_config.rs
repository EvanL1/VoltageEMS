...
use alarmsrv::config_new::{AlarmServiceConfig, generate_default_config};
use std::fs;
use std::path::Path;
...
async fn main() -> Result<()> {
    ...
    match AlarmConfig::load().await {
        Ok(old_config) => {
            ...
            println!("  Redis: {}:{}", old_config.redis.host, old_config.redis.port);
            println!("  API: {}:{}", old_config.api.host, old_config.api.port);
            println!("  Storage retention: {} days", old_config.storage.retention_days);
            ...
            println!("  export ALARMSRV_STORAGE__RETENTION_DAYS={}", old_config.storage.retention_days);
            println!("  export ALARMSRV_STORAGE__AUTO_CLEANUP={}", old_config.storage.auto_cleanup);
            println!("  export ALARMSRV_STORAGE__CLEANUP_INTERVAL_HOURS={}", old_config.storage.cleanup_interval_hours);
        }
        Err(_) => {
