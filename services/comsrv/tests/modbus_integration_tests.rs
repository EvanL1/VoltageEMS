...
use comsrv::core::config::config_manager::{
    ChannelConfig, ChannelParameters, ConfigManager, ProtocolType,
};
use comsrv::core::protocols::common::ProtocolFactory;
use comsrv::core::protocols::modbus::client::{
    ModbusClient, ModbusClientConfig, ModbusCommunicationMode,
};
...
async fn test_modbus_tcp_protocol_creation() {
    ...
    let config_manager =
        ConfigManager::from_file(&config_path).expect("Failed to load test config");

    let channels = config_manager.get_channels();
    let tcp_channel = channels
        .iter()
    ...
    assert!(
        result.is_ok(),
        "Failed to create Modbus TCP protocol: {:?}",
        result.err()
    );
...
async fn test_configuration_validation() {
    ...
    assert!(
        result.is_err(),
        "RTU configuration validation should fail for missing port"
    );
}
...
fn test_modbus_client_config_conversion() {
    ...
    tcp_params.insert(
        "timeout".to_string(),
        serde_yaml::Value::Number(serde_yaml::Number::from(3000)),
    );
    tcp_params.insert(
        "slave_id".to_string(),
    ...
    rtu_params.insert(
        "port".to_string(),
        serde_yaml::Value::String("/dev/ttyUSB0".to_string()),
    );
    rtu_params.insert(
        "baud_rate".to_string(),
        serde_yaml::Value::Number(serde_yaml::Number::from(19200)),
    );
    rtu_params.insert(
        "data_bits".to_string(),
    ...
    rtu_params.insert(
        "parity".to_string(),
        serde_yaml::Value::String("Even".to_string()),
    );
    rtu_params.insert(
        "timeout".to_string(),
...
async fn test_multiple_channels() {
    ...
    assert!(
        created_count >= 1,
        "At least one channel should be created successfully"
    );
...
async fn test_async_operations() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        for i in 0..3 {
            ...
            let task = async move {
                ...
                params.insert(
                    "port".to_string(),
                    serde_yaml::Value::Number(serde_yaml::Number::from(502 + i)),
                );
                params.insert(
                    "timeout".to_string(),
                    serde_yaml::Value::Number(serde_yaml::Number::from(5000)),
    ...
    assert!(
        success_count >= 1,
        "At least one async operation should succeed"
    );
}
