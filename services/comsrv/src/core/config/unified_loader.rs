...
use super::types::{CombinedPoint, TableConfig};
...
impl UnifiedCsvLoader {
    ...
    pub fn load_channel_tables(
        ...
        info!(
            "Successfully loaded {} combined points",
            combined_points.len()
        );
        Ok(combined_points)
    }
    ...
    fn load_modbus_mappings(path: &Path) -> Result<HashMap<u32, ModbusMapping>> {
        ...
        for (idx, result) in reader.records().enumerate() {
            ...
            let bit_position = record.get(6).and_then(|s| {
                if s.is_empty() {
                    None
                } else {
                    s.parse::<u8>().ok()
    ...
    fn combine_point(telemetry: FourTelemetryPoint, mapping: &ModbusMapping) -> CombinedPoint {
        ...
        protocol_params.insert(
            ...
            format!(
                "{}:{}:{}",
                mapping.slave_id, mapping.function_code, mapping.register_address
            ),
        );
