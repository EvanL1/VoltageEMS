    ...
    async fn fetch_config(&self, service_name: &str) -> Result<ConfigResponse> {
        ...
        if response.status().is_success() {
            let config: ConfigResponse = response.json().await
                .map_err(|e| ComSrvError::ConfigError(
                    format!("Failed to parse config response: {}", e)
                ))?;
            ...
            Err(ComSrvError::ConfigError(
                format!("Config fetch failed with status: {}", response.status())
            ))
        }
    }
    
    async fn fetch_item(&self, service_name: &str, key: &str) -> Result<ConfigItemResponse> {
        let url = format!("{}/api/v1/config/service/{}/item/{}", 
        ...
        let response = request
            ...
            .await
            .map_err(|e| ComSrvError::ConfigError(
                format!("Failed to fetch config item: {}", e)
            ))?;
        
        if response.status().is_success() {
            let item: ConfigItemResponse = response.json().await
                .map_err(|e| ComSrvError::ConfigError(
                    format!("Failed to parse config item response: {}", e)
                ))?;
            
            Ok(item)
        } else {
            Err(ComSrvError::ConfigError(
                format!("Config item fetch failed with status: {}", response.status())
            ))
        }
    }
...
impl ConfigCache {
    ...
    pub async fn load(&self, service_name: &str) -> Result<ConfigResponse> {
        ...
        let config: ConfigResponse = serde_json::from_value(cache_entry["config"].clone())
            .map_err(|e| ComSrvError::ConfigError(
                format!("Failed to extract config from cache: {}", e)
            ))?;
