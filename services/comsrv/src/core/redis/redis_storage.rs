...
/// Redis connection manager with enhanced features and connection pooling
#[derive(Clone)]
pub struct RedisConnectionManager {
    client: Client,
    config: RedisConfig,
    /// Connection pool for reuse
    conn_pool: Arc<RwLock<Vec<Connection>>>,
    /// Key prefix cache
...
impl KeyPrefixCache {
    fn new() -> Self {
        Self {
            channel_prefix: "comsrv:channel:".to_string(),
            config_prefix: "comsrv:config:".to_string(),
            stats_prefix: "comsrv:stats:".to_string(),
            pool_prefix: "comsrv:pool:".to_string(),
        }
    }

    fn channel_key(&self, channel_id: u16, suffix: &str) -> String {
        format!("{}{}{suffix}", self.channel_prefix, channel_id)
    }

    fn config_key(&self, config_type: &str) -> String {
        format!("{}{config_type}", self.config_prefix)
    }

    fn stats_key(&self, stats_type: &str) -> String {
        format!("{}{stats_type}", self.stats_prefix)
    }

    fn pool_key(&self, protocol: &str, address: &str) -> String {
        format!("{}{}.{address}", self.pool_prefix, protocol)
    }
}
...
impl RedisConnectionManager {
    ...
    pub async fn new(config: &RedisConfig) -> Result<Self> {
        ...
        let client = Client::open(url.clone())
            .map_err(|e| ComSrvError::RedisError(format!("Invalid Redis URL '{}': {e}", url)))?;
        ...
        Ok(Self {
            client,
            config: config.clone(),
            conn_pool: Arc::new(RwLock::new(Vec::with_capacity(10))),
            key_cache: Arc::new(KeyPrefixCache::new()),
        })
    }
...
impl RedisStore {
    ...
    pub async fn set_realtime_value_with_expire(
        ...
        for attempt in 1..=self.manager.config().max_retries {
            match self.manager.get_connection().await {
                Ok(mut conn) => {
                    match conn
                        .set_ex::<&str, String, ()>(key, val_str.clone(), expire_secs as u64)
                        .await
                    {
                        Ok(_) => {
                            if attempt > 1 {
    ...
    // ========== Point Definition Operations ==========

    /// Store point definition in Redis
    pub async fn set_point_definition(
        &self,
        channel_id: u16,
        point_id: &str,
        definition: &serde_json::Value,
    ) -> Result<()> {
        let key = format!("comsrv:channel:{}:point:{}", channel_id, point_id);
        let val_str = serde_json::to_string(definition).map_err(|e| {
            ComSrvError::RedisError(format!("Serialize point definition error: {e}"))
        })?;
        ...
        conn.set::<&str, String, ()>(&key, val_str)
            .await
            .map_err(|e| {
                ComSrvError::RedisError(format!("Redis set point definition error: {e}"))
            })?;

        // Also add point ID to channel's point set for easy enumeration
        let set_key = format!("comsrv:channel:{}:points", channel_id);
        conn.sadd::<&str, &str, ()>(&set_key, point_id)
            .await
            .map_err(|e| ComSrvError::RedisError(format!("Redis sadd point ID error: {e}")))?;

        Ok(())
    }

    /// Get point definition from Redis
    pub async fn get_point_definition(
        &self,
        channel_id: u16,
        point_id: &str,
        ...
        let val: Option<String> = conn.get(&key).await.map_err(|e| {
            ComSrvError::RedisError(format!("Redis get point definition error: {e}"))
        })?;

        if let Some(json_str) = val {
            let parsed = serde_json::from_str(&json_str).map_err(|e| {
                ComSrvError::RedisError(format!("Deserialize point definition error: {e}"))
            })?;
            Ok(Some(parsed))
        } else {
            Ok(None)
    ...
    /// Store all point definitions for a channel
    pub async fn set_channel_points(
        &self,
        channel_id: u16,
        points: &[crate::core::config::types::channel::CombinedPoint],
    ) -> Result<()> {
        if points.is_empty() {
            return Ok(());
        }
        ...
        // Clear existing point sets for all telemetry types
        let telemetry_types = ["Measurement", "Signal", "Control", "Adjustment"];
        for ttype in &telemetry_types {
            let set_key = format!("comsrv:channel:{}:{}:points", channel_id, ttype);
            pipe.del(&set_key);
        }
        let all_points_key = format!("comsrv:channel:{}:points", channel_id);
        pipe.del(&all_points_key);

        for point in points {
            // 使用 config 前缀来明确这是配置数据而非实时数据
            let key = format!(
                "comsrv:config:channel:{}:{}:{}",
                channel_id, point.telemetry_type, point.point_id
            );
            // 优化字段顺序：重要的标识字段放在前面
            ...
            pipe.set(&key, val_str);

            // Add to type-specific set
            let type_set_key = format!(
                "comsrv:channel:{}:{}:points",
                channel_id, point.telemetry_type
            );
            pipe.sadd(&type_set_key, point.point_id.to_string());

            // Also add to the general points set for backward compatibility
            pipe.sadd(&all_points_key, point.point_id.to_string());
        }
        ...
        let _: () = pipe
            .query_async(&mut conn)
            .await
            .map_err(|e| ComSrvError::RedisError(format!("Redis batch set points error: {e}")))?;

        // 创建点位ID到通道信息的索引，便于快速查找
        let mut index_pipe = Pipeline::new();
        for point in points {
            let index_key = format!("comsrv:index:point:{}", point.point_id);
            let index_value = format!("{}:{}", channel_id, point.telemetry_type);
            index_pipe.set(&index_key, index_value);
        }

        let _: () = index_pipe
            .query_async(&mut conn)
            .await
            .map_err(|e| ComSrvError::RedisError(format!("Redis set point index error: {e}")))?;

        tracing::info!(
            "Stored {} point definitions and indexes for channel {} in Redis",
            points.len(),
            channel_id
        );
        self.manager.return_connection(conn).await;
    ...
    /// Get all point IDs for a channel
    pub async fn get_channel_point_ids(&self, channel_id: u16) -> Result<Vec<String>> {
        let set_key = format!("comsrv:channel:{}:points", channel_id);
        let mut conn = self.manager.get_connection().await?;

        let point_ids: Vec<String> = conn
            .smembers(&set_key)
            .await
            .map_err(|e| ComSrvError::RedisError(format!("Redis smembers error: {e}")))?;

        self.manager.return_connection(conn).await;
        Ok(point_ids)
    }

    /// Get real-time value for a specific point using Hash
    pub async fn get_realtime_point_value(
        &self,
        channel_id: u16,
        telemetry_type: &str,
        point_id: u32,
        ...
        let field = format!("{}:{}", telemetry_type, point_id);
    ...
    /// Get all real-time values for a channel using Hash (optimized for performance)
    pub async fn get_channel_realtime_values(
        &self,
        channel_id: u16,
        ...
        // 使用HGETALL一次获取所有数据
        let all_values: HashMap<String, String> = conn
            .hgetall(&hash_key)
            .await
        ...
        // 解析field名称并构建结果
        for (field, value_str) in all_values {
            // field格式: "telemetry_type:point_id"
            if let Some((_, point_id_str)) = field.split_once(':') {
    ...
    /// Batch update real-time values for a channel using Hash
    pub async fn batch_update_realtime_values(
        &self,
        channel_id: u16,
        values: &HashMap<String, String>, // field -> json_value
    ) -> Result<()> {
        if values.is_empty() {
            return Ok(());
        }
        ...
        // 设置过期时间（可选）
        let ttl = std::time::Duration::from_secs(3600); // 1小时过期
        pipe.expire(&hash_key, ttl.as_secs() as i64);

        let _: () = pipe
            .query_async(&mut conn)
            .await
            .map_err(|e| ComSrvError::RedisError(format!("Redis batch update error: {e}")))?;

        self.manager.return_connection(conn).await;
        Ok(())
    }

    /// Get real-time values by telemetry type using HSCAN
    pub async fn get_realtime_values_by_type(
        &self,
        channel_id: u16,
        telemetry_type: &str,
    ) -> Result<HashMap<u32, serde_json::Value>> {
        let hash_key = format!("comsrv:realtime:channel:{}", channel_id);
        let pattern = format!("{}:*", telemetry_type);
        let mut conn = self.manager.get_connection().await?;

        let mut result = HashMap::new();
        let mut cursor = 0u64;

        // 使用HSCAN分批扫描Hash，避免阻塞
        loop {
            let (new_cursor, items): (u64, Vec<(String, String)>) = redis::cmd("HSCAN")
                .arg(&hash_key)
                .arg(cursor)
                .arg("MATCH")
                .arg(&pattern)
                ...
                .query_async(&mut conn)
                .await
                .map_err(|e| ComSrvError::RedisError(format!("Redis HSCAN error: {e}")))?;

            // 解析结果
            for (field, value_str) in items {
                if let Some((_, point_id_str)) = field.split_once(':') {
                    if let Ok(point_id) = point_id_str.parse::<u32>() {
                        if let Ok(value) = serde_json::from_str::<serde_json::Value>(&value_str) {
                            result.insert(point_id, value);
    ...
    /// Get multiple real-time values by point specifications using HMGET
    pub async fn get_realtime_values_by_specs(
        &self,
        channel_id: u16,
        point_specs: &[(String, u32)], // (telemetry_type, point_id)
    ) -> Result<HashMap<u32, serde_json::Value>> {
        if point_specs.is_empty() {
        ...
        // 构建字段列表
        let fields: Vec<String> = point_specs
            .iter()
            .map(|(telemetry_type, point_id)| format!("{}:{}", telemetry_type, point_id))
            .collect();

        // 使用HMGET批量获取
        let values: Vec<Option<String>> = conn
            .hget(&hash_key, &fields)
            .await
            .map_err(|e| ComSrvError::RedisError(format!("Redis HMGET error: {e}")))?;
        ...
        for (i, value_opt) in values.into_iter().enumerate() {
            if let Some(value_str) = value_opt {
                if let Ok(value) = serde_json::from_str::<serde_json::Value>(&value_str) {
                    result.insert(point_specs[i].1, value);
                }
            }
        }
    ...
    /// List all channel IDs using SCAN
    ...
    /// List all configuration names using SCAN instead of KEYS
    ...
    /// List all connection pool keys using SCAN
    ...
    /// List all statistics keys using SCAN
    ...
    pub async fn set_data<T: Serialize>(
        ...
        if let Some(duration) = expire_secs {
            conn.set_ex::<&str, String, ()>(key, val_str, duration.as_secs())
                .await
                .map_err(|e| {
    ...
    pub async fn set_command(&self, channel_id: &str, command: &RemoteCommand) -> Result<()> {
        ...
        let mut conn = self.manager.get_connection().await?;
        // 设置指令，带过期时间（5分钟）
        conn.set_ex::<&str, String, ()>(&command_key, command_str, 300u64)
            .await
    ...
    pub async fn get_command(
        ...
        let command_key = format!("cmd:{}:{command_id}", channel_id);
    ...
    pub async fn set_command_result(&self, channel_id: &str, result: &CommandResult) -> Result<()> {
        ...
        let mut conn = self.manager.get_connection().await?;
        // 设置结果，带过期时间（1小时）
        conn.set_ex::<&str, String, ()>(&result_key, result_str, 3600u64)
            .await
    ...
    pub async fn create_pubsub(&self) -> Result<redis::aio::PubSub> {
        let pubsub = self
            ...
            .get_async_pubsub()
            .await
            .map_err(|e| ComSrvError::RedisError(format!("Failed to create PubSub: {e}")))?;
