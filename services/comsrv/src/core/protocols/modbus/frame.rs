    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        if data.len() < 7 {
            return Err(ComSrvError::ProtocolError(
                "Invalid MBAP header length".to_string(),
            ));
        }
    ...
    fn parse_rtu_frame(&mut self, data: &[u8]) -> Result<ParsedFrame> {
        ...
        if let Some(last_time) = self.last_frame_time {
            ...
            if elapsed < self.rtu_frame_gap {
                return Err(ComSrvError::ProtocolError(
                    "RTU frame gap violation".to_string(),
                ));
            }
        }
...
mod tests {
    ...
    fn test_tcp_frame() {
        ...
        assert_eq!(
            parsed.mbap_header.transaction_id,
            frame.mbap_header.transaction_id
        );
    ...
    fn test_rtu_frame_completeness_check() {
        ...
        assert_eq!(
            processor.check_rtu_frame_complete(&incomplete).unwrap(),
            None
        );
        ...
        assert_eq!(
            processor.check_rtu_frame_complete(&complete).unwrap(),
            Some(7)
        );
        ...
        assert_eq!(
            processor.check_rtu_frame_complete(&invalid_crc).unwrap(),
            None
        );
    }
}
