...
impl ModbusProtocolEngine {
    ...
    pub async fn send_optimized_request(
        ...
        // 执行实际请求
        let result = self.send_raw_request(slave_id, function_code, address, quantity, transport).await;
    ...
    async fn send_write_multiple_registers(
        ...
        let values: Vec<u16> = data.chunks(2).map(|chunk| {
            if chunk.len() == 2 {
        ...
        let request_data = self.pdu_processor.build_write_multiple_registers_request(address, &values);
        let transaction_id = self.transaction_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let frame = self.frame_processor.read().await.build_frame(slave_id, request_data, Some(transaction_id));
    ...
    async fn send_write_multiple_coils(
        ...
        let request_data = self.pdu_processor.build_write_multiple_coils_request(address, &values);
        let transaction_id = self.transaction_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let frame = self.frame_processor.read().await.build_frame(slave_id, request_data, Some(transaction_id));
    ...
    /// 转换遥调值
    fn convert_adjustment_value(&self, value: f64, mapping: &ModbusAdjustmentMapping) -> Result<Vec<u8>> {
        // 应用缩放和偏移的逆操作
    ...
    pub async fn get_cache_stats(&self) -> HashMap<String, String> {
        ...
        stats.insert("cache_hits".to_string(), engine_stats.cache_hits.to_string());
        stats.insert("cache_misses".to_string(), engine_stats.cache_misses.to_string());
        
        let hit_rate = if engine_stats.cache_hits + engine_stats.cache_misses > 0 {
            (engine_stats.cache_hits as f64 / (engine_stats.cache_hits + engine_stats.cache_misses) as f64) * 100.0
        } else {
            0.0
        };
