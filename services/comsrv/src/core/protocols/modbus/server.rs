...
use voltage_modbus::{
    ModbusServer as VoltageModbusServer,
    ModbusTcpServer, ModbusTcpServerConfig,
    ModbusRegisterBank, ServerStats as VoltageServerStats,
    ModbusResult as VoltageModbusResult,
};
use voltage_modbus::server::{ModbusRtuServer, ModbusRtuServerConfig};

use crate::utils::error::{ComSrvError, Result};
use crate::core::protocols::common::combase::{ComBase, ComBaseImpl, ChannelStatus, PointData};
use crate::core::config::config_manager::ChannelConfig;
use super::common::{ModbusRegisterMapping, ModbusRegisterType, ModbusDataType, ByteOrder};
...
impl ModbusServer {
    ...
    fn initialize_register_bank(register_bank: &Arc<ModbusRegisterBank>, mappings: &[ModbusRegisterMapping]) {
    ...
    pub async fn update_register(&self, address: u16, value: u16) -> Result<()> {
        self.register_bank.write_single_register(address, value)
            .map_err(|e| ComSrvError::CommunicationError(format!("Failed to update register: {}", e)))?;
        Ok(())
    }
    
    /// Update a coil value (for simulation)
    pub async fn update_coil(&self, address: u16, value: bool) -> Result<()> {
        self.register_bank.write_single_coil(address, value)
            .map_err(|e| ComSrvError::CommunicationError(format!("Failed to update coil: {}", e)))?;
        Ok(())
    }
    
    /// Update an input register value (for simulation)
    pub async fn update_input_register(&self, address: u16, value: u16) -> Result<()> {
        self.register_bank.set_input_register(address, value)
            .map_err(|e| ComSrvError::CommunicationError(format!("Failed to update input register: {}", e)))?;
        Ok(())
    }
    
    /// Update a discrete input value (for simulation)
    pub async fn update_discrete_input(&self, address: u16, value: bool) -> Result<()> {
        self.register_bank.set_discrete_input(address, value)
            .map_err(|e| ComSrvError::CommunicationError(format!("Failed to update discrete input: {}", e)))?;
        Ok(())
    }
    ...
    fn create_tcp_server(&self) -> Result<ModbusTcpServer> {
        ...
        let config = ModbusTcpServerConfig {
            ...
            request_timeout: self.config.request_timeout.unwrap_or(Duration::from_secs(30)),
            register_bank: Some(self.register_bank.clone()),
        };
        
        ModbusTcpServer::with_config(config)
            .map_err(|e| ComSrvError::CommunicationError(format!("Failed to create TCP server: {}", e)))
    }
    ...
    fn create_rtu_server(&self) -> Result<ModbusRtuServer> {
        ...
        ModbusRtuServer::with_config(config)
            .map_err(|e| ComSrvError::CommunicationError(format!("Failed to create RTU server: {}", e)))
    }
    ...
    async fn start_internal_server(&mut self) -> Result<()> {
        ...
        let mut internal_server = match self.config.mode {
            ModbusServerMode::Tcp => {
                log::info!("Starting Modbus TCP server on {}:{}", 
                    self.config.bind_address.as_deref().unwrap_or("0.0.0.0"),
                    self.config.bind_port.unwrap_or(502)
                );
        ...
        internal_server.start().await
            .map_err(|e| ComSrvError::CommunicationError(format!("Failed to start server: {}", e)))?;
        ...
        log::info!("Modbus server started successfully in {:?} mode", self.config.mode);
        Ok(())
    }
    ...
    async fn stop_internal_server(&mut self) -> Result<()> {
        if let Some(ref mut server) = self.internal_server {
            server.stop().await
                .map_err(|e| ComSrvError::CommunicationError(format!("Failed to stop server: {}", e)))?;
        }
...
impl std::fmt::Debug for ModbusServer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ModbusServer")
            ...
            .field("is_running", &self.internal_server.as_ref().map(|s| s.is_running()).unwrap_or(false))
            .finish()
    }
}
...
impl ComBase for ModbusServer {
    ...
    fn get_parameters(&self) -> HashMap<String, String> {
        ...
        if let Some(ref timeout) = self.config.request_timeout {
            params.insert("request_timeout_ms".to_string(), timeout.as_millis().to_string());
        }
        
        params.insert("register_mappings_count".to_string(), 
    ...
    async fn start(&mut self) -> Result<()> {
        ...
        match result {
            Ok(_) => {
                log::info!("Modbus server '{}' started in {:?} mode", self.name(), self.config.mode);
                Ok(())
            }
            Err(e) => {
                // If server start failed, stop base service
                self.base.stop().await?;
                self.base.set_error(&format!("Failed to start server: {}", e)).await;
                Err(e)
            }
        }
    }
...
mod tests {
    ...
    async fn test_modbus_server_lifecycle() {
        ...
        assert!(result.is_ok(), "Server should stop successfully: {:?}", result);
        assert!(!server.is_running().await);
    }
    ...
    async fn test_modbus_server_points_collection() {
        ...
        if !server.is_running().await {
            panic!("Server failed to start properly after {} retries", max_retries);
        }
    ...
    fn test_modbus_server_config_modes() {
        ...
        let tcp_server = ModbusServer::new(
            "TCPServer".to_string(),
        ...
        let rtu_server = ModbusServer::new(
            "RTUServer".to_string(),
            channel_config,
            rtu_config,
        );
