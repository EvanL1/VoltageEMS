...
impl Transport for MockTransport {
    ...
    async fn connect(&mut self) -> std::result::Result<(), TransportError> {
        ...
        if !self.config.connect_success {
            ...
            return Err(TransportError::ConnectionFailed("Mock connection failed".to_string()));
        }
    ...
    async fn send(&mut self, data: &[u8]) -> std::result::Result<usize, TransportError> {
        ...
        if self.config.fail_after_operations > 0 
            ...
            warn!("[MockTransport] Send failed - operation limit reached: {}", state.operation_count);
            return Err(TransportError::SendFailed("Operation limit exceeded".to_string()));
        }
        
        // Check message size
        if data.len() > self.config.max_message_size {
            warn!("[MockTransport] Send failed - message too large: {} > {}", data.len(), self.config.max_message_size);
        ...
        if self.config.latency_ms > 0 {
            debug!("[MockTransport] Simulating latency: {}ms", self.config.latency_ms);
            drop(state);
    ...
    async fn receive(
        ...
        if self.config.fail_after_operations > 0 
            && state.operation_count > self.config.fail_after_operations {
            warn!("[MockTransport] Receive failed - operation limit reached: {}", state.operation_count);
            return Err(TransportError::ReceiveFailed("Operation limit exceeded".to_string()));
        }
        
        // Get next response from queue
        if let Some(response) = state.response_queue.pop_front() {
            if response.len() > buffer.len() {
                warn!("[MockTransport] Receive failed - buffer too small: {} > {}", response.len(), buffer.len());
                return Err(TransportError::ReceiveFailed("Buffer too small".to_string()));
            }
            
            // Simulate latency
            if self.config.latency_ms > 0 {
                debug!("[MockTransport] Simulating receive latency: {}ms", self.config.latency_ms);
                drop(state);
                tokio::time::sleep(tokio::time::Duration::from_millis(self.config.latency_ms)).await;
                state = self.state.lock().await;
            }
