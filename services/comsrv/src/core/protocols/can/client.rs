...
pub trait CanClient: ComBase {
    ...
    async fn receive_frames(&self, filter_ids: Option<&[u32]>, timeout_ms: u64) -> Result<Vec<CanFrame>>;
    ...
    async fn write_data(&self, mapping: &CanMessageMapping, value: &serde_json::Value) -> Result<()>;
...
impl CanClientBase {
    ...
    pub fn new(name: &str, config: ChannelConfig) -> Self {
        ...
        let interface_type = config.parameters.get("interface")
            .and_then(|v| v.as_str().map(|s| s.to_string()))
            .map(|s| match s.as_str() {
                s if s.starts_with("socketcan:") => {
                    CanInterfaceType::SocketCan(s.strip_prefix("socketcan:").unwrap_or("can0").to_string())
                },
                s if s.starts_with("peak:") => {
                    CanInterfaceType::PeakCan(s.strip_prefix("peak:").unwrap_or("PCAN_USBBUS1").to_string())
                },
                s if s.starts_with("virtual:") => {
                    CanInterfaceType::Virtual(s.strip_prefix("virtual:").unwrap_or("vcan0").to_string())
                },
        ...
        // Parse bit rate
        let bit_rate = config.parameters.get("bit_rate")
            .and_then(|v| v.as_u64())
            .map(|rate| match rate {
        ...
        let timeout_ms = config.parameters.get("timeout")
            .and_then(|v| v.as_u64())
            .unwrap_or(1000);
    ...
    pub async fn load_message_mappings(&self, mappings: Vec<CanMessageMapping>) {
        ...
        *msg_mappings = mappings;
        ...
        info!("Loaded {} CAN message mappings with {} unique IDs", 
              msg_mappings.len(), filters.len());
    }
    ...
    pub fn extract_data(&self, frame: &CanFrame, mapping: &CanMessageMapping) -> Result<serde_json::Value> {
        ...
        let raw_value = match config.data_type {
            ...
            CanDataType::UInt8 => {
                serde_json::Value::Number(
                    serde_json::Number::from(data[config.start_byte as usize])
                )
            },
            CanDataType::Int8 => {
            ...
            CanDataType::UInt16 => {
                if (config.start_byte as usize + 1) >= data.len() {
                    return Err(ComSrvError::InvalidData("Insufficient data for UInt16".to_string()));
                }
                let val = match config.byte_order {
                    CanByteOrder::BigEndian => {
                        u16::from_be_bytes([
                            data[config.start_byte as usize],
                            data[config.start_byte as usize + 1]
                        ])
                    },
                    CanByteOrder::LittleEndian => {
                        u16::from_le_bytes([
                            data[config.start_byte as usize],
                            data[config.start_byte as usize + 1]
                        ])
            ...
            CanDataType::UInt32 => {
                if (config.start_byte as usize + 3) >= data.len() {
                    return Err(ComSrvError::InvalidData("Insufficient data for UInt32".to_string()));
                }
                let val = match config.byte_order {
                    CanByteOrder::BigEndian => {
                        u32::from_be_bytes([
            ...
            CanDataType::Float32 => {
                if (config.start_byte as usize + 3) >= data.len() {
                    return Err(ComSrvError::InvalidData("Insufficient data for Float32".to_string()));
                }
                let val = match config.byte_order {
                    CanByteOrder::BigEndian => {
                        f32::from_be_bytes([
            ...
            CanDataType::Raw => {
                ...
                serde_json::Value::Array(
                    raw_data.iter().map(|&b| serde_json::Value::Number(serde_json::Number::from(b))).collect()
                )
            },
            CanDataType::String(max_len) => {
                ...
                let null_pos = string_data.iter().position(|&b| b == 0).unwrap_or(string_data.len());
                let string_bytes = &string_data[..null_pos];
                
                match String::from_utf8(string_bytes.to_vec()) {
                    Ok(s) => serde_json::Value::String(s),
                    Err(_) => serde_json::Value::String(
                        string_bytes.iter().map(|&b| format!("{:02X}", b)).collect::<Vec<_>>().join("")
                    ),
                }
            },
            _ => return Err(ComSrvError::InvalidData(format!("Unsupported data type: {:?}", config.data_type))),
        };
            ...
            serde_json::Value::Number(num) => {
                if let Some(val) = num.as_f64() {
                    ...
                    Ok(serde_json::Value::Number(
                        serde_json::Number::from_f64(scaled_value).unwrap_or(serde_json::Number::from(0))
                    ))
                } else {
                    Ok(raw_value)
                }
...
impl ComBase for CanClientBase {
    ...
    fn get_parameters(&self) -> HashMap<String, String> {
        ...
        params.insert("interface".to_string(), format!("{:?}", self.interface_type));
...
impl CanClient for CanClientBase {
    async fn send_frame(&self, frame: &CanFrame) -> Result<()> {
        ...
        debug!("Sending CAN frame: ID=0x{:X}, len={}", frame.id.raw(), frame.data.len());
    ...
    async fn receive_frames(&self, filter_ids: Option<&[u32]>, _timeout_ms: u64) -> Result<Vec<CanFrame>> {
        // This would be implemented with actual CAN interface
    ...
    async fn write_data(&self, mapping: &CanMessageMapping, value: &serde_json::Value) -> Result<()> {
        ...
        debug!("Writing CAN data for mapping: {}, value: {:?}", mapping.name, value);
...
mod tests {
    ...
    fn create_test_can_config() -> ChannelConfig {
        ...
        parameters.insert("interface".to_string(), serde_yaml::Value::String("socketcan:can0".to_string()));
        parameters.insert("bit_rate".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(500000)));
    ...
    async fn test_can_client_creation() {
        ...
        assert!(matches!(client.interface_type(), CanInterfaceType::SocketCan(_)));
    }
