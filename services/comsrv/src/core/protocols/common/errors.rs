...
impl BaseCommError {
    ...
    pub fn timeout(timeout_ms: u64) -> Self {
        Self::Core(ComSrvError::TimeoutError(format!(
    ...
    pub fn category(&self) -> ErrorCategory {
        match self {
            Self::Core(err) => match err {
                ...
                ComSrvError::ParsingError(_) | ComSrvError::SerializationError(_) => {
                    ErrorCategory::DataConversion
                }
                ComSrvError::ConfigError(_) | ComSrvError::ConfigurationError(_) => {
                ...
                ComSrvError::InvalidOperation(_) | ComSrvError::StateError(_) => {
                    ErrorCategory::State
                }
    ...
    pub fn severity(&self) -> ErrorSeverity {
        match self {
            Self::Core(err) => match err {
                ...
                ComSrvError::TimeoutError(_) | ComSrvError::IoError(_) => ErrorSeverity::Low,
...
mod tests {
    ...
    fn test_protocol_specific_error() {
        let modbus_err =
            BaseCommError::protocol_specific("modbus_exception", "Illegal function code");
        assert_eq!(modbus_err.category(), ErrorCategory::ProtocolSpecific);
        assert!(!modbus_err.is_retriable());

        if let BaseCommError::ProtocolSpecific {
            error_type,
            message,
        } = modbus_err
        {
    ...
    fn test_error_category_display() {
        ...
        assert_eq!(
            ErrorCategory::ProtocolSpecific.to_string(),
            "protocol_specific"
        );
    }
