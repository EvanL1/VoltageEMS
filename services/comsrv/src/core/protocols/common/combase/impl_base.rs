...
use super::point_manager::{UniversalPointConfig, UniversalPointManager};
...
impl ComBaseImpl {
    ...
    pub async fn load_point_configs(&self, configs: Vec<UniversalPointConfig>) -> Result<()> {
        if let Some(ref point_manager) = self.point_manager {
            ...
            info!(
                "Loaded {} point configurations for channel {}",
...
impl ComBase for ComBaseImpl {
    ...
    fn get_parameters(&self) -> HashMap<String, String> {
        ...
        params.insert(
            "has_point_manager".to_string(),
            self.has_point_manager().to_string(),
        );

        if let Some(ref point_manager) = self.point_manager {
            params.insert(
                "point_manager_channel".to_string(),
                point_manager.channel_id().to_string(),
            );
        }
    ...
    async fn get_diagnostics(&self) -> HashMap<String, String> {
        ...
        if let Some(ref point_manager) = self.point_manager {
            ...
            diagnostics.insert(
                "enabled_points".to_string(),
                stats.enabled_points.to_string(),
            );
            diagnostics.insert(
                "read_operations".to_string(),
                stats.read_operations.to_string(),
            );
            diagnostics.insert(
                "write_operations".to_string(),
                stats.write_operations.to_string(),
            );
            diagnostics.insert(
                "validation_errors".to_string(),
                stats.validation_errors.to_string(),
            );
            diagnostics.insert(
                "last_point_update".to_string(),
                stats.last_update.to_rfc3339(),
            );
        }
...
mod tests {
    ...
    async fn test_diagnostics_with_point_manager() {
        ...
        assert_eq!(
            diagnostics.get("has_point_manager"),
            Some(&"true".to_string())
        );
    ...
    async fn test_unified_data_access_interface() {
        let config = create_test_config();
        let service =
            ComBaseImpl::new_with_point_manager("Unified Test Service", "unified_protocol", config);
        ...
        let signaling_points = service
            .get_enabled_points_by_type(&TelemetryType::Signaling)
            .await;
        assert_eq!(signaling_points.len(), 2);

        let control_points = service
            .get_enabled_points_by_type(&TelemetryType::Control)
            .await;
        assert_eq!(control_points.len(), 1);
        assert_eq!(control_points[0], "3001");

        let setpoint_points = service
            .get_enabled_points_by_type(&TelemetryType::Setpoint)
            .await;
        ...
        assert_eq!(
            stats.points_by_type.get(&TelemetryType::Telemetry),
            Some(&2)
        );
        assert_eq!(
            stats.points_by_type.get(&TelemetryType::Signaling),
            Some(&2)
        );
    ...
    async fn test_legacy_protocol_compatibility() {
        ...
        let enabled_points = service
            .get_enabled_points_by_type(&TelemetryType::Telemetry)
            .await;
        assert!(enabled_points.is_empty());

        let telemetry_data = service
            .get_points_by_telemetry_type(&TelemetryType::Telemetry)
            .await;
