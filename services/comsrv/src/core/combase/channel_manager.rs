...
use crate::core::combase::channel::{ChannelEntry, ChannelStats};
use crate::core::combase::point_config::RuntimeConfigProvider;
use crate::core::combase::sync::TelemetrySync;
...
async fn create_protocol_client(
    ...
    match protocol_name {
        ...
        #[cfg(feature = "can")]
        "can" => {
            use crate::protocols::can::CanProtocol;
            Ok(Box::new(CanProtocol::from_runtime_config(runtime_config)?))
        },
        "virtual" => {
...
impl ChannelManager {
    ...
    async fn create_command_trigger(
        ...
        Option<Arc<RwLock<crate::core::combase::trigger::CommandTrigger>>>,
        tokio::sync::mpsc::Receiver<crate::core::combase::traits::ChannelCommand>,
    )> {
