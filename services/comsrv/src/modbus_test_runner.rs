...
pub async fn test_modbus_pdu_basic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let read_request = processor.build_read_request(ModbusFunctionCode::Read03, 40001, 10);
    ...
    println!(
        "  Parsed request - Start: {}, Quantity: {}",
        parsed.start_address, parsed.quantity
    );
...
pub async fn test_protocol_engine() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Initial stats - Cache hits: {}, Cache misses: {}",
...
pub async fn test_modbus_client_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Config created - Channel: {}, Timeout: {}ms",
        config.channel_name,
    ...
    println!(
        "  Stats - Total: {}, Success: {}, Failed: {}, Avg time: {:.1}ms",
