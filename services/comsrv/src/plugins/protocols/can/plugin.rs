...
impl ProtocolPlugin for CanPlugin {
    ...
    async fn create_instance(&self, channel_config: ChannelConfig) -> Result<Box<dyn ComBase>> {
        ...
        {
            use crate::core::transport::can::{CanBitRate, CanFilter, CanTransportConfig};

            let factory = TransportFactory::new();
            let transport_config = CanTransportConfig {
                name: format!("can_{}", channel_config.id),
                interface: interface.clone(),
                bit_rate: CanBitRate::Custom(bitrate),
                can_fd: use_fd,
        ...
        #[cfg(not(all(target_os = "linux", feature = "can")))]
        {
            return Err(Error::ProtocolNotSupported(
                "CAN protocol is only supported on Linux with the 'can' feature enabled"
                    .to_string(),
            ));
        }
