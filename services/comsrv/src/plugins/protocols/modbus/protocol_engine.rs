...
impl ModbusProtocolEngine {
    ...
    pub async fn read_telemetry_point(
        ...
        _transport: &mut dyn crate::core::transport::traits::Transport,
    ) -> Result<PointData> {
        // 遥测点默认使用功能码03（读保持寄存器）
    ...
    async fn send_raw_request(
        ...
        _transport.send(&frame).await?;

        // Receive response
        let mut response_buffer = vec![0u8; 1024];
        let response_len = _transport
            .receive(&mut response_buffer, Some(Duration::from_secs(5)))
            .await?;
        let response = response_buffer[..response_len].to_vec();
        ...
        // Extract data
        match pdu_result {
            crate::plugins::protocols::modbus::pdu::PduParseResult::Response(response) => {
                debug!(
            ...
            crate::plugins::protocols::modbus::pdu::PduParseResult::Exception(exception) => {
                warn!(
