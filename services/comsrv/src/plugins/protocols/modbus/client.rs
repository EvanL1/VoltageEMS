...
impl ModbusClient {
    ...
    pub async fn new(config: ModbusChannelConfig, _transport: Box<dyn Transport>) -> Result<Self> {
        // 创建传输桥接
        let transport = Arc::new(Mutex::new(_transport));
    ...
    pub async fn start_polling(&mut self) -> Result<()> {
        info!("[{}] start_polling called", self.config.channel_name);
        ...
        if self.polling_engine.is_none() {
            ...
            info!(
                "[{}] Polling engine already exists, reusing it",
                self.config.channel_name
            );
        }
                            ...
                            Box::pin(async move {
                                ...
                                match engine_clone
                                    .send_optimized_request(
                                        ...
                                        address,
                                        quantity,
                                        &mut *transport_clone.lock().await,
                                    )
                                    .await
                                {
                                    Ok(data) => {
    ...
    async fn create_modbus_points(&self) -> Result<Vec<ModbusPoint>> {
        ...
        for (point_id, mapping) in &mappings.telemetry_mappings {
            let point = ModbusPoint {
                ...
                data_format: mapping.data_type.clone(), // Use data_type field
                register_count: match mapping.data_type.as_str() {
                    "float32" | "int32" | "uint32" => 2,
                    "float64" | "int64" | "uint64" => 4,
                    _ => 1,
                },
                byte_order: Some("ABCD".to_string()), // Default to big-endian
            };
            points.push(point);
        }
    ...
    pub async fn disconnect(&self) -> Result<()> {
        ...
        match self.transport.lock().await.disconnect().await {
            Ok(_) => {
                state.connected = false;
                info!(
        ...
        match telemetry_type {
            TelemetryType::Telemetry => {
                if let Some(mapping) = mappings.telemetry_mappings.get(&point_id) {
                    self.protocol_engine
                        .read_telemetry_point(mapping, &mut *self.transport.lock().await)
                        .await
            ...
            TelemetryType::Signal => {
                if let Some(mapping) = mappings.signal_mappings.get(&point_id) {
                    self.protocol_engine
                        .read_signal_point(mapping, &mut *self.transport.lock().await)
                        .await
                        .map(|pd| CommonPointData {
                            id: pd.id,
                            name: pd.name,
                            ...
                            unit: pd.unit,
                            description: pd.description,
                            telemetry_type: Some(TelemetryType::Signal),
                            channel_id: Some(self.config.channel_id),
                        })
                } else {
                    Err(ComSrvError::NotFound(format!(
    ...
    pub async fn write_point(&self, point_id: u32, value: &str) -> Result<()> {
        ...
        if let Some(mapping) = mappings.adjustment_mappings.get(&point_id) {
            ...
            return self
                .protocol_engine
                .write_adjustment_point(mapping, float_value, &mut *self.transport.lock().await)
                .await;
        }
        ...
        if let Some(mapping) = mappings.control_mappings.get(&point_id) {
            ...
            return self
                .protocol_engine
                .execute_control_point(mapping, bool_value, &mut *self.transport.lock().await)
                .await;
        }
        ...
        match telemetry_type {
            TelemetryType::Adjustment => {
                if let Some(mapping) = mappings.adjustment_mappings.get(&point_id) {
                    ...
                    self.protocol_engine
                        .write_adjustment_point(
                            mapping,
                            float_value,
                            &mut *self.transport.lock().await,
                        )
                        .await
                } else {
                    Err(ComSrvError::NotFound(format!(
    ...
    pub async fn health_check(&self) -> Result<HashMap<String, String>> {
        ...
        // 检查传输层状态
        let transport_connected = self.transport.lock().await.is_connected().await;
        health.insert(
...
impl ComBase for ModbusClient {
    ...
    async fn start(&mut self) -> Result<()> {
        // Create channel-specific span for all operations
    ...
    async fn read_point(&self, point_id: &str) -> Result<CommonPointData> {
        // 支持两种格式：
        // 1. 旧格式：直接的point_id (需要自动检测类型)
        // 2. 新格式："telemetry_type:point_id" 例如 "Measurement:1"
    ...
    async fn get_diagnostics(&self) -> HashMap<String, String> {
        ...
        // 传输层诊断
        let transport_diag = self.transport.lock().await.diagnostics().await;
