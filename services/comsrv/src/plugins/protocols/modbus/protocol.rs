...
impl ComBase for ModbusProtocol {
    ...
    async fn initialize(&mut self, channel_config: Arc<ChannelConfig>) -> Result<()> {
        ...
        voltage_libs::log_to_channel!(
            channel_config.id,
            tracing::Level::INFO,
            "Step 2: Processing {} configured points ({} telemetry, {} signal, {} control, {} adjustment)", 
        ...
        voltage_libs::log_to_channel!(
            channel_config.id,
            tracing::Level::INFO,
            "Step 3: Setting up {} points in storage (T:{}, S:{}, C:{}, A:{})",
            total_points,
        ...
        *self.adjustment_points.write().await = adjustment_modbus_points;
        ...
        voltage_libs::log_to_channel!(
            channel_config.id,
            tracing::Level::INFO,
            "Step 3 completed: Successfully configured {} total points",
            total_points
        );
...
impl ComClient for ModbusProtocol {
    ...
    async fn connect(&mut self) -> Result<()> {
        voltage_libs::log_to_channel!(
            self.channel_id,
            tracing::Level::INFO,
            "Connection Phase: Starting connection to Modbus device"
        );
        ...
        voltage_libs::log_to_channel!(
            self.channel_id,
            tracing::Level::INFO,
            "Connection phase completed successfully"
        );
