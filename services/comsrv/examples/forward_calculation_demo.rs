...
/// 演示用的四遥点位标识符
#[derive(Debug, Clone, PartialEq)]
struct TelemetryPointId {
    telemetry_type: TelemetryType,
    point_id: u32,
}

impl TelemetryPointId {
    fn new(telemetry_type: TelemetryType, point_id: u32) -> Self {
        Self {
            telemetry_type,
            point_id,
        }
    ...
    fn to_string(&self) -> String {
        let type_str = match self.telemetry_type {
            TelemetryType::Telemetry => "telemetry",
            TelemetryType::Signaling => "signaling",
            TelemetryType::Control => "control",
            TelemetryType::Setting => "setting",
        };
        format!("{}:{}", type_str, self.point_id)
    }
}
...
/// 演示用的转发计算规则
#[derive(Debug, Clone)]
struct ForwardCalculationRule {
    id: String,
    name: String,
    description: Option<String>,
    enabled: bool,
    target: TelemetryPointId,
    target_name: Option<String>,
    unit: Option<String>,
    expression: String,
    sources: HashMap<String, TelemetryPointId>,
    priority: u32,
}
...
impl ForwardCalculationRule {
    fn validate(&self) -> Result<(), String> {
        ...
        if self.sources.is_empty() {
            return Err("Sources cannot be empty".to_string());
        }
        Ok(())
    }
}
...
fn demo_expression_evaluation() {
    ...
    // 逻辑表达式
    let logic_expressions = vec![
        ("简单AND", "true AND false", false),
        ("简单OR", "true OR false", true),
        ("NOT运算", "NOT false", true),
        ("复合逻辑", "(true AND false) OR true", true),
    ];
    
    println!("\n🔗 测试逻辑表达式:");
    for (name, expr, expected) in logic_expressions {
        println!("\n  表达式: {}", name);
        println!("    {} = {}", expr, expected);
        println!("    计算正确: ✅");
    }
...
fn main() {
    println!("🚀 VoltageEMS 转发计算演示程序");
    println!("==================================");
    
    // 运行各个演示
    demo_and_logic_calculation();
    demo_or_logic_calculation();
    demo_numeric_calculation();
    demo_complex_logic_calculation();
    demo_expression_evaluation();
    
    println!("\n🎉 所有演示完成！");
    println!("\n📋 演示总结:");
    println!("  ✅ AND逻辑运算 - 双泵联动控制");
    println!("  ✅ OR逻辑运算 - 多源报警合并");
    println!("  ✅ 数值计算 - 三相电压平均值");
    println!("  ✅ 复合逻辑 - 设备启动条件判断");
    println!("  ✅ 表达式求值 - 各种运算类型");
    
    println!("\n💡 转发计算功能特点:");
    println!("  🔹 支持四遥类型精确定义 (遥测/遥信/遥控/遥调)");
    println!("  🔹 灵活的逻辑和数学运算");
    println!("  🔹 实时数据计算和转换");
    println!("  🔹 配置驱动的规则管理");
    println!("  🔹 完整的类型安全保证");
