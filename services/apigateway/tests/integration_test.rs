...
use voltage_common::types::{PointValue, PointData, Quality};
...
async fn test_api_gateway_health() -> anyhow::Result<()> {
    ...
    let api_gateway_url = std::env::var("API_GATEWAY_URL")
        .unwrap_or_else(|_| "http://apigateway:8089".to_string());
    ...
    if response.status().is_success() {
        ...
        println!("⚠️ API Gateway health check returned: {}", response.status());
