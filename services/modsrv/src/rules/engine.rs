...
impl ExecutionContext {
    ...
    pub async fn load_device_status(&mut self) -> Result<()> {
        let keys = self.store.get_keys("ems:device:status:*")?;
        
        for key in keys {
            if let Ok(status_json) = self.store.get_string(&key) {
    ...
    pub fn get_device_parameter(&self, device_id: &str, parameter: &str) -> Result<Value> {
        if let Some(status) = self.device_status.get(device_id) {
            if let Some(params) = status.get("parameters") {
        ...
        Err(ModelSrvError::DeviceError(format!(
            "Parameter {} not found for device {}", parameter, device_id
        )))
    }
    
    /// Set a variable for rule execution
    ...
    pub async fn execute_device_action(&self, device_id: &str, operation: &str, parameters: &Value) -> Result<Value> {
        ...
        self.store.set_string(&cmd_key, &command.to_string())?;
        
        if let Some(mut redis) = self.store.get_redis_connection() {
            redis.push_list("ems:control:queue", &cmd_id)?;
        }
