...
impl TemplateManager {
    ...
    pub fn load_template_index(&self) -> Result<TemplateIndex> {
        ...
        let (index_path, is_yaml) = if yaml_index_path.exists() {
            ...
            return Err(ModelSrvError::TemplateError(format!(
                "Template index file not found in directory: {}",
                self.templates_dir.display()
            )));
        };

        let index_content = fs::read_to_string(&index_path).map_err(|e| {
            ModelSrvError::TemplateError(format!("Failed to read template index: {}", e))
        })?;

        let index: TemplateIndex = if is_yaml {
            serde_yaml::from_str(&index_content).map_err(|e| {
                ModelSrvError::TemplateError(format!("Failed to parse YAML template index: {}", e))
            })?
        } else {
            serde_json::from_str(&index_content).map_err(|e| {
                ModelSrvError::TemplateError(format!("Failed to parse JSON template index: {}", e))
            })?
        };
    ...
    pub fn load_template(
        ...
        let content =
            fs::read_to_string(template_path).map_err(|e| ModelSrvError::IoError(e.to_string()))?;

        // Determine format based on file extension
        let model_with_actions = match template_path.extension().and_then(|ext| ext.to_str()) {
            Some("yaml") | Some("yml") => serde_yaml::from_str::<ModelWithActions>(&content)
                .map_err(|e| ModelSrvError::YamlError(e.to_string()))?,
            Some("json") => serde_json::from_str::<ModelWithActions>(&content)
    ...
    pub fn get_template_by_id(&self, template_id: &str) -> Result<TemplateInfo> {
        ...
        let entries =
            fs::read_dir(templates_dir).map_err(|e| ModelSrvError::IoError(e.to_string()))?;
        ...
        Err(ModelSrvError::TemplateError(format!(
            "Template not found: {}",
            template_id
        )))
    }
    ...
    pub fn list_templates(&self) -> Result<Vec<TemplateInfo>> {
        let templates_dir = Path::new(&self.templates_dir);
        println!(
            "Looking for templates in directory: {}",
    ...
    fn load_template_info(&self, path: &Path) -> Result<TemplateInfo> {
        let file_content =
            fs::read_to_string(path).map_err(|e| ModelSrvError::IoError(e.to_string()))?;
    ...
    pub fn create_instance<T: DataStore>(
        ...
        let template = templates
            ...
            .ok_or_else(|| {
                ModelSrvError::TemplateError(format!("Template not found: {}", template_id))
            })?;
        ...
        if let Some(name) = instance_name {
            ...
            instance_content =
                instance_content.replace("{{name}}", &format!("Instance of {}", template.name));
        }

        // Parse the JSON content
        let model_with_actions: serde_json::Value =
            match template_path.extension().and_then(|ext| ext.to_str()) {
                Some("yaml") | Some("yml") => {
    ...
    fn instantiate_template(
        ...
        Ok(ModelWithActions { model, actions })
    }
...
impl TemplateService {
    ...
    pub fn delete_template(&self, template_id: &str) -> Result<()> {
        ...
        let filtered_templates: Vec<String> = templates
            .into_iter()
