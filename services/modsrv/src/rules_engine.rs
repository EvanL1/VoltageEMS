...
impl ExecutionContext {
        ...
        match handler {
            Some(handler) => {
                ...
                let result = handler
                    ...
                    .await
                    .map_err(|e| {
                        ModelSrvError::ActionExecutionError(format!(
            ...
            None => Err(ModelSrvError::ActionTypeNotSupported(format!(
    ...
    pub fn evaluate_expression(&self, expression: &str) -> Result<bool> {
        ...
        if expression.contains("==") {
            ...
            if parts.len() != 2 {
                return Err(ModelSrvError::RuleError(format!(
                    "Invalid expression: {}",
                    expression
                )));
            }
            ...
            match (left.as_f64(), right.as_f64()) {
                ...
                _ => Err(ModelSrvError::RuleError(format!(
                    "Cannot compare: {} and {}",
                    left, right
                ))),
            }
    ...
    pub fn resolve_variable(&self, name: &str) -> Result<Value> {
        ...
        if name.starts_with("$") {
            ...
            match self.get_variable(var_name) {
                ...
                None => Err(ModelSrvError::RuleError(format!(
                    "Variable not found: {}",
                    var_name
                ))),
            }
...
pub fn build_rule_graph(rule_def: DagRule) -> Result<RuntimeRule> {
    ...
    for edge_def in &rule_def.edges {
        let from_idx = node_map.get(&edge_def.from).ok_or_else(|| {
            ModelSrvError::RuleError(format!("Node not found: {}", edge_def.from))
        })?;
    ...
    if toposort(&graph, None).is_err() {
        return Err(ModelSrvError::RuleError(
    ...
    match node.definition.node_type {
        NodeType::Input => {
            ...
            let device_id = node
                ...
                .ok_or_else(|| {
                    ModelSrvError::RuleError("Input node missing device_id".to_string())
                })?;
            ...
            let parameter = node
                ...
                .and_then(Value::as_str)
                .ok_or_else(|| {
                    ModelSrvError::RuleError("Input node missing parameter".to_string())
                })?;
        ...
        NodeType::Condition => {
            ...
            let expression = node
                ...
                .and_then(Value::as_str)
                .ok_or_else(|| {
                    ModelSrvError::RuleError("Condition node missing expression".to_string())
                })?;
        ...
        NodeType::Transform => {
            ...
            let transform_type = node
                ...
                .and_then(Value::as_str)
                .ok_or_else(|| {
                    ModelSrvError::RuleError("Transform node missing transform_type".to_string())
                })?;
            ...
            match transform_type {
                "scale" => {
                    ...
                    if let Some(num) = value.as_f64() {
                        ...
                        Err(ModelSrvError::RuleError(format!(
        ...
        NodeType::Aggregate => {
            ...
            let aggregation_type = node
                ...
                .and_then(Value::as_str)
                .ok_or_else(|| {
                    ModelSrvError::RuleError("Aggregate node missing aggregation_type".to_string())
                })?;
            ...
            let inputs = node
                ...
                .ok_or_else(|| {
                    ModelSrvError::RuleError("Aggregate node missing inputs".to_string())
                })?;
...
impl RuleExecutor {
    ...
    pub fn register_action_handler<T: ActionHandler + Send + Sync + 'static>(
        ...
        let mut handlers = self
            .action_handlers
            .lock()
            .map_err(|_| ModelSrvError::LockError)?;
