...
use crate::engine::{SimpleRule, SimpleRuleEngine};
use crate::redis::RedisStore;

/// Simple API state for the simplified rule engine
pub struct SimpleApiState {
    pub engine: Arc<RwLock<SimpleRuleEngine>>,
...
/// List all simple rules
pub async fn list_simple_rules(
    State(state): State<Arc<SimpleApiState>>,
    ...
    let result = async {
        ...
        Ok::<Vec<SimpleRule>, anyhow::Error>(rules)
    }
    .await;
...
/// Get a specific simple rule
pub async fn get_simple_rule(
    State(state): State<Arc<SimpleApiState>>,
    ...
    let result = async {
        ...
        match state.store.get_string(&rule_key).await? {
            Some(rule_json) => {
                let rule: SimpleRule = serde_json::from_str(&rule_json)?;
                Ok(rule)
            }
...
/// Create simple rule request
#[derive(Debug, Deserialize)]
pub struct CreateSimpleRuleRequest {
    pub rule: SimpleRule,
}

/// Create a new simple rule
pub async fn create_simple_rule(
    State(state): State<Arc<SimpleApiState>>,
    Json(request): Json<CreateSimpleRuleRequest>,
) -> impl IntoResponse {
    let result = async {
        ...
        info!("Created simple rule: {} ({})", rule.name, rule.id);
        Ok(rule)
    }
    .await;
...
/// Update simple rule request
#[derive(Debug, Deserialize)]
pub struct UpdateSimpleRuleRequest {
    pub rule: SimpleRule,
}

/// Update an existing simple rule
pub async fn update_simple_rule(
    State(state): State<Arc<SimpleApiState>>,
    Path(rule_id): Path<String>,
    Json(request): Json<UpdateSimpleRuleRequest>,
) -> impl IntoResponse {
    let result = async {
        let rule = request.rule;
        ...
        info!("Updated simple rule: {} ({})", rule.name, rule.id);
        Ok(rule)
    }
    .await;
...
/// Delete a simple rule
pub async fn delete_simple_rule(
    State(state): State<Arc<SimpleApiState>>,
    Path(rule_id): Path<String>,
) -> impl IntoResponse {
    let result = async {
        ...
        info!("Deleted simple rule: {}", rule_id);
...
pub async fn execute_simple_rule(
    State(state): State<Arc<SimpleApiState>>,
    Path(rule_id): Path<String>,
    Json(_request): Json<ExecuteSimpleRuleRequest>,
) -> impl IntoResponse {
    let result = async {
        debug!("Executing simple rule: {}", rule_id);
...
/// Test a simple rule without saving
pub async fn test_simple_rule(
    State(state): State<Arc<SimpleApiState>>,
    Json(request): Json<TestSimpleRuleRequest>,
) -> impl IntoResponse {
    let result = async {
        let rule = request.rule;

        debug!("Testing simple rule: {}", rule.name);
...
pub async fn get_simple_rule_history(
    State(state): State<Arc<SimpleApiState>>,
    Path(rule_id): Path<String>,
...
/// Get rule statistics
pub async fn get_simple_rule_stats(
    State(state): State<Arc<SimpleApiState>>,
    Path(rule_id): Path<String>,
) -> impl IntoResponse {
    let result = async {
...
/// Health check endpoint for simple engine
pub async fn simple_health_check(State(state): State<Arc<SimpleApiState>>) -> impl IntoResponse {
