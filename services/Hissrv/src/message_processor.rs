...
impl MessageProcessor {
    ...
    pub async fn init_storage(&mut self, storage: Arc<Mutex<InfluxDBStorage>>) -> Result<()> {
        ...
        info!("Initialized batch writer with buffer size: {}, flush interval: {}s",
    ...
    fn parse_channel(&self, channel: &str) -> Result<ChannelInfo> {
        ...
        if parts.len() < 2 {
            return Err(HisSrvError::ParseError(
                format!("Invalid channel format: {}", channel)
            ));
        }
            ...
            if let Some(values) = obj.get("values").and_then(|v| v.as_object()) {
                ...
                for (point_id, value) in values {
                    ...
                    points.push(DataPoint {
                        ...
                        timestamp,
                        tags: HashMap::from([
                            ("channel_id".to_string(), channel_info.channel_id.to_string()),
