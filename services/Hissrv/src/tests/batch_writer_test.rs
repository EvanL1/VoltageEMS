...
use crate::tests::test_utils::{create_test_batch, create_test_data_point};
...
impl BatchWriter for TestBatchWriter {
    async fn write_batch(&mut self, points: &[DataPoint]) -> Result<()> {
        ...
        if *count < self.fail_count {
            *count += 1;
            return Err(crate::error::HisSrvError::WriteError(format!(
                "Test failure {}/{}",
...
async fn test_batch_writer_retry_mechanism() {
    ...
    // 添加数据触发刷新
    buffer
        .add(create_test_data_point("metric_1", 1.0))
        .await
        .unwrap();
    buffer
        .add(create_test_data_point("metric_2", 2.0))
        .await
        .unwrap();
...
async fn test_batch_writer_statistics() {
    ...
    for batch in 0..3 {
        for i in 0..5 {
            buffer
                .add(create_test_data_point(
                    &format!("metric_{}_{}", batch, i),
                    i as f64,
                ))
                .await
                .unwrap();
