...
use crate::types::{PointData, PointId, PointValue, Quality};
use chrono::Utc;
...
pub fn generate_test_point_data(point_id: PointId, value: f64) -> PointData {
    PointData {
        point_id,
...
/// Generate a batch of test point data
pub fn generate_test_point_batch(start_id: PointId, count: usize) -> Vec<PointData> {
    (0..count)
        .map(|i| {
            let point_id = start_id + i as u32;
            let value = (i as f64).sin() * 100.0;
            generate_test_point_data(point_id, value)
        })
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn test_generate_point_data() {
        let data = generate_test_point_data(1, 42.0);
        assert_eq!(data.point_id, 1);
        assert_eq!(data.value, PointValue::Float(42.0));
        assert_eq!(data.quality, Quality::Good);
    }
