...
impl RoutingCache {
    ...
    pub fn from_maps(
        ...
        // Initialize C2M routing
        let c2m = Arc::new(DashMap::new());
        for (k, v) in c2m_data {
            c2m.insert(k, v);
        }

        // Initialize M2C routing
        let m2c = Arc::new(DashMap::new());
        for (k, v) in m2c_data {
            m2c.insert(k, v);
        }

        // Initialize C2C routing
    ...
    /// }
    /// ```
    pub fn lookup_c2m(&self, key: &str) -> Option<String> {
        self.c2m.get(key).map(|v| v.value().clone())
    }
    ...
    /// }
    /// ```
    pub fn lookup_m2c(&self, key: &str) -> Option<String> {
        self.m2c.get(key).map(|v| v.value().clone())
    }
    ...
    /// }
    /// ```
    pub fn lookup_c2c(&self, key: &str) -> Option<String> {
        self.c2c.get(key).map(|v| v.value().clone())
    }

    /// Insert C2C routing entry
    pub fn insert_c2c(&self, source_key: String, target_key: String) {
        self.c2c.insert(source_key, target_key);
    }

    /// Remove C2C routing entry
    pub fn remove_c2c(&self, source_key: &str) -> Option<(String, String)> {
        self.c2c.remove(source_key)
    }

    /// Get all C2C routing entries matching a prefix
    pub fn get_c2c_by_prefix(&self, prefix: &str) -> Vec<(String, String)> {
        self.c2c
            .iter()
    ...
    pub fn get_c2m_by_prefix(&self, prefix: &str) -> Vec<(String, String)> {
        self.c2m
            .iter()
            .filter(|entry| entry.key().starts_with(prefix))
            .map(|entry| (entry.key().clone(), entry.value().clone()))
    ...
    pub fn get_m2c_by_prefix(&self, prefix: &str) -> Vec<(String, String)> {
        self.m2c
            .iter()
            .filter(|entry| entry.key().starts_with(prefix))
            .map(|entry| (entry.key().clone(), entry.value().clone()))
...
mod tests {
    ...
    fn test_from_maps() {
        ...
        assert_eq!(cache.lookup_c2m("2:T:1"), Some("23:M:1".to_string()));
        assert_eq!(cache.lookup_m2c("23:A:4"), Some("2:A:1".to_string()));
        assert_eq!(cache.lookup_c2c("1001:T:1"), Some("1002:T:5".to_string()));
    ...
    fn test_update() {
        ...
        assert_eq!(cache.lookup_c2m("2:T:1"), Some("23:M:1".to_string()));
        assert_eq!(cache.lookup_m2c("23:A:4"), Some("2:A:1".to_string()));
        assert_eq!(cache.lookup_c2c("1001:S:2"), Some("1002:S:3".to_string()));
    }
    ...
    fn test_c2c_operations() {
        ...
        // Verify lookups
        assert_eq!(cache.lookup_c2c("1001:S:2"), Some("1003:S:1".to_string()));
        assert_eq!(cache.lookup_c2c("2001:A:1"), Some("2002:C:3".to_string()));
        ...
        assert_eq!(
            removed,
            Some(("1001:T:1".to_string(), "1002:T:5".to_string()))
        );
