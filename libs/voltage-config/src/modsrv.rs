...
/// Calculations table record
/// Stores virtual/computed point definitions from YAML configuration
#[allow(dead_code)]
#[derive(Schema)]
#[table(name = "calculations", suffix = "CHECK(output_type IN ('M','A'))")]
struct CalculationsRecord {
    #[column(primary_key, autoincrement)]
    calculation_id: i64,
    ...
    #[column(not_null)]
    calculation_type: String, // JSON serialized CalculationType

    #[column(not_null)]
    output_inst: u16, // Output instance ID

    #[column(not_null)]
    output_type: String, // M=Measurement, A=Action

    #[column(not_null)]
    output_id: u32, // Output point ID
...
/// Calculations table SQL (generated by Schema macro)
pub const CALCULATIONS_TABLE: &str = CalculationsRecord::CREATE_TABLE_SQL;
...
pub use crate::common::PointRole as PointType;
...
impl ConfigValidator for ModsrvConfig {
...
pub struct ModsrvValidator {
    config: Option<ModsrvConfig>,
    raw_yaml: Option<serde_yaml::Value>,
}

impl ModsrvValidator {
    ...
    pub fn from_config(config: ModsrvConfig) -> Self {
        Self {
            config: Some(config),
            raw_yaml: None,
        }
    ...
    pub fn from_file(path: &std::path::Path) -> Result<Self> {
        ...
        let config = serde_yaml::from_str::<ModsrvConfig>(&content).map_err(|e| {
...
///
/// let instance_id = 100u16;
///
/// // Address strings for routing tables
/// let addr_m = RedisKeys::measurement(instance_id, 1);
/// assert_eq!(addr_m, "inst:100:M:1");
/// let addr_a = RedisKeys::action(instance_id, 2);
/// assert_eq!(addr_a, "inst:100:A:2");
///
/// // Runtime storage keys (hash)
...
/// products, instances, and point mappings. These queries ensure
/// consistency across the service and simplify schema updates.
///
/// # Query Categories
///
/// - **Existence checks**: Verify instances/products exist
/// - **Counting**: Count products and instances
/// - **Loading**: Retrieve full records with properties
/// - **Mappings**: Query point routing configurations
///
/// # Example
/// ```rust
/// use voltage_config::modsrv::ModsrvQueries;
/// use sqlx::SqlitePool;
///
/// async fn check_instance(pool: &SqlitePool, id: &str) -> bool {
///     sqlx::query_scalar(ModsrvQueries::CHECK_INSTANCE_EXISTS)
///         .bind(id)
///         .fetch_one(pool)
///         .await
///         .unwrap_or(false)
...
impl ModsrvQueries {
    ...
    ///
    /// Returns boolean (1 or 0) indicating whether the instance exists.
    /// Bind parameter: instance_id
    ...
    ///
    /// Returns boolean (1 or 0) indicating whether the instance name exists.
    /// Bind parameter: instance_name
...
///
/// Centralized table names used across the model service.
/// These constants prevent typos and make schema refactoring easier.
///
/// # Tables
///
/// - **products**: Product templates with hierarchy
/// - **instances**: Runtime instances of products
/// - **measurement_points**: M-type point definitions
/// - **action_points**: A-type point definitions
/// - **property_templates**: Product property schemas
/// - **measurement_routing**: Channel to instance measurement routing
/// - **action_routing**: Instance to channel action routing
///
/// # Example
/// ```rust
/// use voltage_config::modsrv::TableNames;
///
/// // Build query with table name
/// let query = format!(
///     "SELECT * FROM {} WHERE product_name = ?",
///     TableNames::PRODUCTS
/// );
/// ```
...
///
/// @trait SqlInsertableProduct
/// @purpose Auto-insert product point definitions without manual field mapping
/// @implementors MeasurementPoint, ActionPoint, PropertyTemplate
/// @philosophy Structures know how to persist themselves (ultrathink)
...
pub trait SqlInsertableProduct {
    ...
    /// @input product_name: &str - Product name this point belongs to
    /// @output Result<SqliteQueryResult, sqlx::Error> - Rows affected and last insert ID
    /// @throws sqlx::Error - Foreign key constraints, duplicate primary keys
    /// @side-effects Inserts into respective tables (measurement_points, action_points, property_templates)
    /// @example
    /// ```ignore
    /// let point = MeasurementPoint { point_id: 1, name: "DC Voltage".into(), /* other fields */ };
    /// point.insert_with(&pool, "pv_inverter").await?;
    /// ```
...
/// @implementor MeasurementPoint for SqlInsertableProduct
/// @purpose Insert measurement point definitions
...
/// @implementor ActionPoint for SqlInsertableProduct
/// @purpose Insert action point definitions for control operations
/// @table action_points
...
/// @implementor PropertyTemplate for SqlInsertableProduct
/// @purpose Insert property templates for instance configuration
