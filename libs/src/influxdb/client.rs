...
impl InfluxClient {
    ...
    pub fn from_config(config: InfluxConfig) -> Result<Self> {
        tracing::debug!("Creating InfluxDB client: url={}, org={}, bucket={}", 
                       config.url, config.org, config.get_bucket());
    ...
    pub async fn write_line_protocol(&self, data: &str) -> Result<()> {
        ...
        tracing::debug!("Writing to InfluxDB: org={}, bucket={}, data_len={}", 
                       org, bucket, data.len());
