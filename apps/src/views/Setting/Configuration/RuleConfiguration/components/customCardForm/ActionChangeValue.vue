      ...
      <div class="main-section">
        ...
        <div class="section rule-section">
          <div class="section__header">
            ...
            <el-button class="section__add-btn" type="primary" link @click="addRuleRow">
              <el-icon><Plus /></el-icon>
            </el-button>
          </div>
...
const varPointOptions = ref<Record<number, Array<{ label: string; value: number | string }>>>({})
...
function resolveInstanceId(raw: number | string): number {
  ...
  const id = Number(found?.d?.instance_id ?? found?.value)
...
onMounted(() => {
  fetchInstances().then(async () => {
    normalizeVariables()
    // 回显：为已选 instance + pointType 的变量加载并缓存点位，不清空已选 point
    const vars = Array.isArray(cardData.value?.config?.variables)
      ? cardData.value.config.variables
      : []
