...
function getQualityType(quality: number): string {
  if (quality >= 192) return 'success'
  if (quality >= 128) return 'warning'
  return 'danger'
}

function getQualityText(quality: number): string {
...
function confirmControl() {
  ...
  ElMessage.success(`Control command sent to point ${controlForm.value.pointId}`)
